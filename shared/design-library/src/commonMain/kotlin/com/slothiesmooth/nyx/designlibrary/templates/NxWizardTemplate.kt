    ...
    Column(modifier = modifier.fillMaxSize().background(colors.bg)) {
        ...
        Column(
            modifier = Modifier
                .fillMaxWidth()
                .weight(1f)
                // imePadding before verticalScroll shrinks the scroll viewport above the keyboard, so
                // the focused field scrolls into view instead of being covered (edge-to-edge means the
                // window no longer resizes for the IME; nothing else consumes WindowInsets.ime).
                .imePadding()
