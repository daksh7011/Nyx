...
/**
 * Stores stego PNG bytes as `<root>/<id>.png`. [root] is the desktop vault directory (mirrors
 * android). The directory is (re)created before every write because kotlinx-io's sink does not
 * create missing parents, so a fresh install would otherwise fail the first save.
 */
